...
- **`request.html`** — staff-facing request form. Staff can submit a request and see the current waiting list only. No timing/ETA is shown, no delete capability, and it blocks a duplicate request for a location that's already active.
...
The restrictions on `request.html` (no delete, no timing) are enforced in the page itself — there's no button for it and no timing data is rendered. Since the Firebase rules are currently open (`.read`/`.write`: true for anyone), this is a UI-level restriction, not a hard permission wall — someone who deliberately dug into the raw database URL could still see or change data directly. For a one-day event with people using the page normally, this is a reasonable tradeoff. If you ever want a real access wall, that needs Firebase Authentication, which is a bigger addition — just let me know if that becomes worth it.
