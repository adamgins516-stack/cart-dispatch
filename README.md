...
- **`index.html`** — staff-facing request form (this is the simple link you hand out — it's the site root). Staff can submit a request and see the current waiting list, how long each has been waiting, and an estimated wait time per request. No delete capability, and it blocks a duplicate request for a location that's already active.
